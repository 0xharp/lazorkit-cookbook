            ...
            <div className={`${theme.bgCardAlt} rounded-2xl p-6`}>
              ...
              <div className={`space-y-4 text-sm ${theme.textSecondary}`}>
                ...
                <div className={`${theme.bgCard} rounded-lg p-4`}>
                  <p className={`font-semibold ${theme.textPrimary} mb-2`}>The Best of Both Worlds:</p>
                ...
                <div className={theme.statusWarning + " rounded-lg p-4"}>
                  <p className="font-semibold mb-2">Key Integration Point:</p>
                  <pre className={`text-xs ${theme.codeBlock} text-gray-100 rounded p-2 overflow-x-auto`}>
    ...
    paymasterConfig: {
      paymasterUrl: 'https://kora.devnet.lazorkit.com',
            ...
            <div className={`${theme.bgCard} rounded-2xl p-6`}>
              <h2 className={`text-xl font-bold ${theme.textPrimary} mb-4`}>How It Works</h2>
              <div className={`space-y-4 text-sm ${theme.textSecondary}`}>
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <p className={`font-semibold ${theme.textPrimary}`}>Register LazorKit</p>
                    <p className={theme.textMuted}>Call <code className={`${theme.textAccent}`}>registerLazorkitWallet()</code> on app init to make LazorKit discoverable</p>
                  </div>
                </div>
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <p className={`font-semibold ${theme.textPrimary}`}>Setup Wallet Provider</p>
                    <p className={theme.textMuted}>Wrap your app with the wallet adapter provider of your choice</p>
                  </div>
                </div>
                <div className="flex items-start gap-3">
                  <span className={`${theme.infoPurple} ${theme.infoPurpleTitle} rounded-full w-6 h-6 flex items-center justify-center flex-shrink-0 text-xs font-bold`}>3</span>
                  <div>
                    <p className={`font-semibold ${theme.textPrimary}`}>LazorKit Appears as Option</p>
                    <p className={theme.textMuted}>Users see LazorKit alongside Phantom, Solflare, and other wallets</p>
                  </div>
                </div>
                <div className="flex items-start gap-3">
                  <span className={`${theme.infoPurple} ${theme.infoPurpleTitle} rounded-full w-6 h-6 flex items-center justify-center flex-shrink-0 text-xs font-bold`}>4</span>
                  <div>
                    <p className={`font-semibold ${theme.textPrimary}`}>Use Standard Hooks</p>
                    <p className={theme.textMuted}>Use the adapter's hooks (useWallet, etc.) - works the same regardless of which wallet is connected</p>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="space-y-6 w-full min-w-0">
            <div className={`${theme.bgCard} rounded-2xl p-6`}>
              <h2 className={`text-xl font-bold ${theme.textPrimary} mb-6`}>Choose an Adapter</h2>
              ...
              <div className="space-y-4">
                {ADAPTERS.map((adapter) => (
                  <Link
                    key={adapter.id}
                    href={`/examples/05-wallet-adapter-integration/${adapter.id}`}
                    className={`block ${theme.bgCardAlt} rounded-xl p-4 hover:opacity-90 transition-all group`}
                  >
                    <div className="flex items-center gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between mb-1">
                          <h3 className={`text-lg font-semibold ${theme.textPrimary} group-hover:text-purple-400 transition-colors`}>
                            {adapter.name}
                          </h3>
                          <span className={`${theme.textAccent} group-hover:translate-x-1 transition-transform`}>→</span>
                        </div>
                        ...
                        <div className="flex flex-wrap gap-2 mb-3">
                          {adapter.packages.map((pkg) => (
                            <code
                              key={pkg}
                              className={`text-xs ${theme.codeBlock} px-2 py-1 rounded`}
                            >
                              {pkg}
                            </code>
                          ))}
